(define (domain Pickup_and_Place_Hard)
...
(:types
	...
	cobblestone_stairs grass_path water planks grass leaves log farmland cobblestone chest torch gravel fence grass_block dirt flower oak_door glass wheat_seeds ladder - item
	cobblestone_stairs-block grass_path-block water-block planks-block grass-block leaves-block log-block farmland-block cobblestone-block chest-block torch-block gravel-block fence-block grass_block-block dirt-block flower-block oak_door-block glass-block wheat_seeds-block ladder-block - destructible-block
)
(:predicates
...
(block-present ?b) (= (x ?b) 3) (= (y ?b) 4) (= (z ?b) -22)))
	 )
	:effect (and 
(goal-achieved ?ag))
)
