...
(:types
	...
	log leaves cobblestone grass_block - item
	log-block leaves-block cobblestone-block grass_block-block - destructible-block
)
(:predicates
...
(:action checkgoal
	:parameters (?ag - agent)
	:precondition (and 
(agent-alive ?ag)   (>= (agent-num-log ?ag) 10))
	:effect (and 
(goal-achieved ?ag))
)
