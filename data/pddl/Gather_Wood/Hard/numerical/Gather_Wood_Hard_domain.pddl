(define (domain Gather_Wood_Hard)
...
(:types
	...
	glass grass_block water oak_door farmland grass_path planks leaves cobblestone dirt grass flower chest ladder log cobblestone_stairs wheat_seeds fence torch gravel - item
	glass-block grass_block-block water-block oak_door-block farmland-block grass_path-block planks-block leaves-block cobblestone-block dirt-block grass-block flower-block chest-block ladder-block log-block cobblestone_stairs-block wheat_seeds-block fence-block torch-block gravel-block - destructible-block
)
(:predicates
...
(:action checkgoal
	:parameters (?ag - agent)
	:precondition (and 
(agent-alive ?ag)   (>= (agent-num-log ?ag) 1))
	:effect (and 
(goal-achieved ?ag))
)
