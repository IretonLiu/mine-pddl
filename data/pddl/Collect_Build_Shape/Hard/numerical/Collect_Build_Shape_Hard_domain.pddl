(define (domain Collect_Build_Shape_Hard)
...
(:types
	...
	gravel cobblestone water farmland grass_path chest torch planks flower wheat_seeds grass ladder grass_block stone fence cobblestone_stairs glass log oak_stairs dirt oak_door leaves - item
	gravel-block cobblestone-block water-block farmland-block grass_path-block chest-block torch-block planks-block flower-block wheat_seeds-block grass-block ladder-block grass_block-block stone-block fence-block cobblestone_stairs-block glass-block log-block oak_stairs-block dirt-block oak_door-block leaves-block - destructible-block
)
(:predicates
	(item-present ?i - item)
