...
(:types
	...
	oak_door ladder flower gravel chest water grass wheat_seeds dirt planks torch cobblestone_stairs grass_block log cobblestone glass fence leaves farmland - item
	oak_door-block ladder-block flower-block gravel-block chest-block water-block grass-block wheat_seeds-block dirt-block planks-block torch-block cobblestone_stairs-block grass_block-block log-block cobblestone-block glass-block fence-block leaves-block farmland-block - destructible-block
)
(:predicates
...
(:action checkgoal
	:parameters (?ag - agent ?b1 - block ?b2 - block ?b3 - block ?b4 - block ?b5 - block)
	:precondition (and 
(agent-alive ?ag)
 (and 
(block-present ?b1)
