...
(:types
	...
	planks glass glowstone dirt fence grass_block cobblestone bricks stone obsidian log oak_stairs stone_bricks - item
	planks-block glass-block glowstone-block dirt-block fence-block grass_block-block cobblestone-block bricks-block stone-block obsidian-block log-block oak_stairs-block stone_bricks-block - destructible-block
)
(:predicates
...
(:action place-bricks-north
	:parameters (?ag - agent ?b - bricks-block ?x - position ?y - position ?y_down - position ?z - position ?z_front - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y)
...
(:action move_and_pickup-bricks-north
	:parameters (?ag - agent ?i - bricks ?x - position ?y_up - position ?y_down - position ?y_2_down - position ?z_start - position ?z_end - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y_down)
...
(:action jumpup_and_pickup-bricks-north
	:parameters (?ag - agent ?i - bricks ?x - position ?y_up - position ?y_up_up - position ?y_down - position ?z_start - position ?z_end - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y_down)
...
(:action jumpdown_and_pickup-bricks-north
	:parameters (?ag - agent ?i - bricks ?x - position ?y_up - position ?y_down - position ?y_2_down - position ?y_3_down - position ?z_start - position ?z_end - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y_down)
...
(:action place-obsidian-north
	:parameters (?ag - agent ?b - obsidian-block ?x - position ?y - position ?y_down - position ?z - position ?z_front - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y)
...
(:action move_and_pickup-obsidian-north
	:parameters (?ag - agent ?i - obsidian ?x - position ?y_up - position ?y_down - position ?y_2_down - position ?z_start - position ?z_end - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y_down)
...
(:action jumpup_and_pickup-obsidian-north
	:parameters (?ag - agent ?i - obsidian ?x - position ?y_up - position ?y_up_up - position ?y_down - position ?z_start - position ?z_end - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y_down)
...
(:action jumpdown_and_pickup-obsidian-north
	:parameters (?ag - agent ?i - obsidian ?x - position ?y_up - position ?y_down - position ?y_2_down - position ?y_3_down - position ?z_start - position ?z_end - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y_down)
...
(:action place-bricks-east
	:parameters (?ag - agent ?b - bricks-block ?x - position ?x_front - position ?y - position ?y_down - position ?z - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y)
...
(:action move_and_pickup-bricks-east
	:parameters (?ag - agent ?i - bricks ?z - position ?x_start - position ?x_end - position ?y_up - position ?y_down - position ?y_2_down - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x_start)
 (at-y ?ag ?y_down)
...
(:action jumpup_and_pickup-bricks-east
	:parameters (?ag - agent ?i - bricks ?z - position ?x_start - position ?x_end - position ?y_up - position ?y_up_up - position ?y_down - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x_start)
 (at-y ?ag ?y_down)
...
(:action jumpdown_and_pickup-bricks-east
	:parameters (?ag - agent ?i - bricks ?z - position ?x_start - position ?x_end - position ?y_up - position ?y_down - position ?y_2_down - position ?y_3_down - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x_start)
 (at-y ?ag ?y_down)
...
(:action place-obsidian-east
	:parameters (?ag - agent ?b - obsidian-block ?x - position ?x_front - position ?y - position ?y_down - position ?z - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x)
 (at-y ?ag ?y)
...
(:action move_and_pickup-obsidian-east
	:parameters (?ag - agent ?i - obsidian ?z - position ?x_start - position ?x_end - position ?y_up - position ?y_down - position ?y_2_down - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x_start)
 (at-y ?ag ?y_down)
...
(:action jumpup_and_pickup-obsidian-east
	:parameters (?ag - agent ?i - obsidian ?z - position ?x_start - position ?x_end - position ?y_up - position ?y_up_up - position ?y_down - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x_start)
 (at-y ?ag ?y_down)
...
(:action jumpdown_and_pickup-obsidian-east
	:parameters (?ag - agent ?i - obsidian ?z - position ?x_start - position ?x_end - position ?y_up - position ?y_down - position ?y_2_down - position ?y_3_down - position ?n_start - count ?n_end - count ?b1 - block)
	:precondition (and 
(agent-alive ?ag)
 (at-x ?ag ?x_start)
 (at-y ?ag ?y_down)
...
(:action checkgoal
	:parameters (?ag - agent ?b1 - block ?b2 - block ?b3 - block ?b4 - block ?b5 - block ?b6 - block ?b7 - block ?b8 - block ?b9 - block ?b10 - block ?b11 - block ?b12 - block ?b13 - block ?b14 - block ?b15 - block ?b16 - block ?b17 - block ?b18 - block ?b19 - block ?b20 - block ?b21 - block ?b22 - block ?b23 - block ?b24 - block ?b25 - block ?b26 - block ?b27 - block ?b28 - block ?b29 - block ?b30 - block ?b31 - block ?b32 - block ?b33 - block ?b34 - block ?b35 - block ?b36 - block ?b37 - block ?b38 - block ?b39 - block ?b40 - block ?b41 - block ?b42 - block ?b43 - block ?b44 - block ?b45 - block ?b46 - block ?b47 - block ?b48 - block ?b49 - block ?b50 - block ?b51 - block ?b52 - block ?b53 - block ?b54 - block ?b55 - block ?b56 - block ?b57 - block ?b58 - block ?b59 - block ?b60 - block ?b61 - block ?b62 - block ?b63 - block ?b64 - block ?b65 - block ?b66 - block ?b67 - block ?b68 - block ?b69 - block ?b70 - block ?b71 - block ?b72 - block ?b73 - block ?b74 - block ?b75 - block ?b76 - block ?b77 - block ?b78 - block ?b79 - block ?b80 - block ?b81 - block ?b82 - block ?b83 - block ?b84 - block ?b85 - block ?b86 - block ?b87 - block ?b88 - block ?b89 - block ?b90 - block ?b91 - block ?b92 - block ?b93 - block ?b94 - block ?b95 - block ?b96 - block ?b97 - block ?b98 - block ?b99 - block ?b100 - block ?b101 - block ?b102 - block ?b103 - block ?b104 - block ?b105 - block ?b106 - block ?b107 - block ?b108 - block ?b109 - block ?b110 - block ?b111 - block ?b112 - block ?b113 - block ?b114 - block ?b115 - block ?b116 - block)
	:precondition (and 
(agent-alive ?ag)
 (and 
(block-present ?b1)
...
(block-present ?b99)
 ...
 (at-z ?b99 position8)
	...
	(and 
(block-present ?b100)
 (at-x ?b100 position-6)
 (at-y ?b100 position4)
 (at-z ?b100 position8)
)
	(and 
(block-present ?b101)
 (at-x ?b101 position-5)
 (at-y ?b101 position4)
 (at-z ?b101 position8)
)
	(and 
(block-present ?b102)
 (at-x ?b102 position-4)
 (at-y ?b102 position4)
 (at-z ?b102 position5)
)
	(and 
(block-present ?b103)
 (at-x ?b103 position-4)
 (at-y ?b103 position4)
 (at-z ?b103 position6)
)
	(and 
(block-present ?b104)
 (at-x ?b104 position-4)
 (at-y ?b104 position4)
 (at-z ?b104 position7)
)
	(and 
(block-present ?b105)
 (at-x ?b105 position-4)
 (at-y ?b105 position4)
 (at-z ?b105 position8)
)
	(and 
(block-present ?b106)
 (at-x ?b106 position-8)
 (at-y ?b106 position6)
 (at-z ?b106 position1)
)
	(and 
(block-present ?b107)
 (at-x ?b107 position-8)
 (at-y ?b107 position6)
 (at-z ?b107 position2)
)
	(and 
(block-present ?b108)
 (at-x ?b108 position-8)
 (at-y ?b108 position6)
 (at-z ?b108 position3)
)
	(and 
(block-present ?b109)
 (at-x ?b109 position-7)
 (at-y ?b109 position6)
 (at-z ?b109 position0)
)
	(and 
(block-present ?b110)
 (at-x ?b110 position-7)
 (at-y ?b110 position6)
 (at-z ?b110 position4)
)
	(and 
(block-present ?b111)
 (at-x ?b111 position-6)
 (at-y ?b111 position6)
 (at-z ?b111 position0)
)
	(and 
(block-present ?b112)
 (at-x ?b112 position-6)
 (at-y ?b112 position6)
 (at-z ?b112 position4)
)
	(and 
(block-present ?b113)
 (at-x ?b113 position-5)
 (at-y ?b113 position6)
 (at-z ?b113 position0)
)
	(and 
(block-present ?b114)
 (at-x ?b114 position-5)
 (at-y ?b114 position6)
 (at-z ?b114 position4)
)
	(and 
(block-present ?b115)
 (at-x ?b115 position-3)
 (at-y ?b115 position4)
 (at-z ?b115 position2)
)
	(and 
(block-present ?b116)
 (at-x ?b116 position-6)
 (at-y ?b116 position4)
 (at-z ?b116 position2)
)
	)
